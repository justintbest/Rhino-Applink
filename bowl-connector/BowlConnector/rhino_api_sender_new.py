# -*- coding: utf-8 -*-
# rhino_api_sender.py
# Collapsible floating panel for the Seating Bowl Generator Rhino Connector.
...
COL_BG       = drawing.Color.FromArgb(26,  26,  26)
COL_SURFACE  = drawing.Color.FromArgb(40,  40,  40)
COL_ACCENT   = drawing.Color.FromArgb(190,  0, 255)
COL_TEXT     = drawing.Color.FromArgb(255, 255, 255)
COL_MUTED    = drawing.Color.FromArgb(160, 160, 160)
COL_HEADER   = drawing.Color.FromArgb(35,  35,  35)
...
class ALineSenderDialog(forms.Form):
    ...
    def __init__(self):
        ...
        self._lbl_title = forms.Label()
        self._lbl_title.Text = "Bowl Connector"
        self._lbl_title.TextColor = COL_TEXT
        self._lbl_title.VerticalAlignment = forms.VerticalAlignment.Center

        self._btn_toggle = forms.Button()
        self._btn_toggle.Text = "Expand"
        self._btn_toggle.BackgroundColor = COL_ACCENT
        self._btn_toggle.TextColor = COL_TEXT
        self._btn_toggle.Size = drawing.Size(80, 22)
        self._btn_toggle.Click += self.on_toggle

        header_layout = forms.TableLayout()
        header_layout.BackgroundColor = COL_HEADER
        header_layout.Padding = drawing.Padding(10, 0)
        header_layout.Spacing = drawing.Size(8, 0)
        header_layout.Rows.Add(forms.TableRow(
            forms.TableCell(self._lbl_title, True),
            forms.TableCell(self._btn_toggle, False),
        ))

        self._header_panel = forms.Panel()
        self._header_panel.BackgroundColor = COL_HEADER
        self._header_panel.Height = COLLAPSED_H
        self._header_panel.Content = header_layout
        ...
        body_layout = forms.DynamicLayout()
        body_layout.Padding = drawing.Padding(20)
        body_layout.Spacing = drawing.Size(0, 10)
        body_layout.DefaultSpacing = drawing.Size(6, 6)
        body_layout.BackgroundColor = COL_BG
        body_layout.AddRow(make_label("Email"))
        body_layout.AddRow(self.txt_email)
        body_layout.AddRow(make_label("Password"))
        body_layout.AddRow(self.txt_password)
        body_layout.AddRow(make_label("A-Line Name"))
        body_layout.AddRow(self.txt_name)
        ...
        body_layout.AddRow(chk_row)
        body_layout.AddRow(self.btn_select)
        body_layout.AddRow(self.lbl_curve_status)
        body_layout.AddRow(self.preview)
        body_layout.AddRow(self.lbl_status)
        ...
        body_layout.AddRow(btn_panel)

        self._body_panel = forms.Panel()
        self._body_panel.BackgroundColor = COL_BG
        self._body_panel.Content = body_layout
        self._body_panel.Visible = False

        outer = forms.DynamicLayout()
        outer.Spacing = drawing.Size(0, 0)
        outer.BackgroundColor = COL_BG
        outer.AddRow(self._header_panel)
        outer.AddRow(self._body_panel)

        self.Content = outer
        self.Size = drawing.Size(PANEL_W, COLLAPSED_H)

    def on_toggle(self, sender, e):
        self._expanded = not self._expanded
        if self._expanded:
            self._body_panel.Visible = True
            self._btn_toggle.Text = "Collapse"
